<div>
    ...
    <div class="toolbox toolbox-pagination justify-content-between">
        <p class="showing-info mb-2 mb-sm-0"> نمایش <span>{{$paginator->firstItem()}} - {{$paginator->lastItem()}} از {{$paginator->total()}}</span>محصول</p>
        <ul class="pagination">
            {{-- Previous Page Link --}}
            ...
            @else
            <li class="prev">
                <a href="javascript:void(0);" dusk="previousPage{{ $paginator->getPageName() == 'page' ? '' : '.' . $paginator->getPageName() }}" aria-label="قبلی " wire:click="previousPage('{{ $paginator->getPageName() }}')" wire:loading.attr="disabled" rel="prev">
                    <i class="w-icon-long-arrow-right"></i>قبلی
                </a>
            </li>
            @endif
            ...
            <li class="next">
                <a href="javascript:void(0);" dusk="nextPage{{ $paginator->getPageName() == 'page' ? '' : '.' . $paginator->getPageName() }}" aria-label="بعدی " wire:click="nextPage('{{ $paginator->getPageName() }}')" wire:loading.attr="disabled" rel="next">
